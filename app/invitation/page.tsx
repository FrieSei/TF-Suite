...
const passwordRequirements: PasswordRequirement[] = [
  { id: "length", label: "At least 8 characters long", validator: (password) => password.length >= 8 },
  { id: "uppercase", label: "Contains uppercase letter", validator: (password) => /[A-Z]/.test(password) },
  { id: "lowercase", label: "Contains lowercase letter", validator: (password) => /[a-z]/.test(password) },
  { id: "number", label: "Contains number", validator: (password) => /\d/.test(password) },
  { id: "special", label: "Contains special character", validator: (password) => /[!@#$%^&*(),.?":{}|<>]/.test(password) },
];
  ...
  const handleSetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/accept-invitation", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, password, userData }),
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-background">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          {userData && <CardDescription>Setting up your account as a {userData.role} at {userData.location}</CardDescription>}
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSetPassword} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {password && confirmPassword && !passwordsMatch && <p className="text-sm text-destructive mt-1">Passwords do not match</p>}
              </div>
            </div>
            <div className="bg-muted p-4 rounded-lg space-y-2">
              ...
              {meetsRequirements.map((req) => (
                <div key={req.id} className="flex items-center gap-2">
                  {req.isMet ? <Check className="h-4 w-4 text-green-500" /> : <div className="h-4 w-4 rounded-full border border-muted-foreground" />}
                  <span className={cn("text-sm", req.isMet ? "text-green-700 dark:text-green-500" : "text-muted-foreground")}>{req.label}</span>
                </div>
              ))}
            </div>
            {status === "success" && (
